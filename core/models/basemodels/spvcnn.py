...
class SPVCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"nan sum (x.F): {torch.sum(torch.isnan(x.F))}")
        z = PointTensor(x.F, x.C.float())
        # print(f"z.F: {z.F}")
        x0 = initial_voxelize(z, self.pres, self.vres)
        # print(f"x0.F: {x0.F}")
        # print(f"nan sum (x0.F): {torch.sum(torch.isnan(x0.F))}")
        x0 = self.stem(x0)
        # print(f"x0.F: {x0.F}")
        nana = torch.sum(torch.isnan(x0.F))
        # print(f"nan sum (x0.F): {nana}")
        # np.save(f'pcl_{nana}', x.F.cpu().detach().numpy())

        z0 = voxel_to_point(x0, z, nearest=False)
        z0.F = z0.F
        # print(f"z0.F: {z0.F}")
        # print(f"(x0, z0).size {(x0.F.size(), z0.F.size())}")
